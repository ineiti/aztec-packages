...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    // two uint256 + 4 bytes for <int>op_type + 4 byte for <int>tag_choice + 4 byte for <int>is_zero
    size_t minimum_size = (2 * 32) + 12;
    ...
    // Zeroes have a habit of triggering edge cases, so we add want a certain proportion of "b"s to be zero
    int chance_of_zero = 5; // 1 in 5 chance of being zero
    bool is_zero = (fuzzed_data.ConsumeIntegralInRange<int>(0, chance_of_zero - 1) == 0);
    if (is_zero) {
        b = MemoryValue::from_tag(b.get_tag(), 0);
    }

    MemoryValue c = MemoryValue::from_tag(MemoryTag::FF, 0); // Placeholder for result
    bool div_0_error = false;                                // For execution trace sel_err
    ...
    try {
        ...
        // Expected alu exception (e.g., division by zero), so just exit early
        return 0;
    }
    ...
    auto trace = TestTraceContainer({ {
        ...
        { avm2::Column::execution_sel_opcode_error, div_0_error ? 1 : 0 },             // = sel_err
