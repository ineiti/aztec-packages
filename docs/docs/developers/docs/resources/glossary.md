...
`aztec` is a CLI tool (with an extensive set of parameters) that enables users to perform a wide range of tasks. It can: run a node, run a local network, execute tests, generate contract interfaces for javascript and more.
