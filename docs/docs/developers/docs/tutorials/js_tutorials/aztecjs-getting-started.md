...
We want to [connect to our running local network](../../aztec-js/how_to_connect_to_local_network.md) and import the test accounts into the local PXE. Let's call them Alice and Bob (of course). Create an `index.ts` with it:
